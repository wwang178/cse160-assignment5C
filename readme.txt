Things I referenced
-The Three.js tutorials
-lobster by God Appeasers [CC-BY] via Poly Pizza (https://poly.pizza/m/25ijpkQ3YAb)
-"Seaweed" (https://skfb.ly/oHV9n) by OwenCalingasan is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
-https://www.tutorialspoint.com/threejs/threejs_geometries.htm
-https://www.tutorialspoint.com/threejs/threejs_sphere_geometry.htm
-https://stackoverflow.com/questions/18923328/is-there-a-way-to-enable-cors-on-github-pages
-https://www.youtube.com/watch?v=D-gkwzExddk
-I also used ChatGPT to fix some CORS issues in order to get it to host properly on Github pages

-https://developer.mozilla.org/en-US/docs/Web/API/setTimeout
-https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/now
-https://stackoverflow.com/questions/20420065/loop-diagonally-through-two-dimensional-array

Notes:
-Switching to another tab/unfocusing this tab causes the rotation delay offsets to get messed up

-https://sbcode.net/threejs/loaders-fbx/
-"Water Plant" (https://skfb.ly/oLGJF) by Norah van Nimwegen is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
-Tuft of grass by Poly by Google [CC-BY] via Poly Pizza
-grass blades by Tiff Eidmann [CC-BY] via Poly Pizza
-https://www.youtube.com/watch?v=QATefHrO4kg
-https://stackoverflow.com/questions/40694372/what-is-the-right-way-to-remove-a-mesh-completely-from-the-scene-in-three-js
-https://www.youtube.com/watch?v=IA3HjAV2nzU
-https://fonts.google.com/specimen/Roboto
-https://threejs.org/docs/#examples/en/loaders/FontLoader
-<a href="https://www.vecteezy.com/free-vector/turquoise">Turquoise Vectors by Vecteezy</a>
-https://www.deviantart.com/antiretrovirus/art/Stock-Underwater-sand-129791658
-https://www.deviantart.com/dadrian/art/Under-water-texture-169170730
-https://stackoverflow.com/questions/37192936/three-js-orbitcontrols-how-to-change-center
-Fish by Quaternius
-Blowfish by jeremy [CC-BY] via Poly Pizza
-Angler Fish by Anonymous [CC-BY] via Poly Pizza
-Manta ray by Quaternius
-https://stackoverflow.com/questions/11127543/how-to-move-an-object-forward-in-three-js
-I used ChatGPT to do things like math for circles and figuring out errors with the fbx loader and animations
-https://discourse.threejs.org/t/shadows-not-working/30269
-Pufferfish by Poly by Google [CC-BY] via Poly Pizza


Notes:
-15 x 15 map, 25 bombs
-Left click to reveal, right click to flag/unflag
-It takes a while to load at the beginning